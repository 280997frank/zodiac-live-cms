...
import { SessionInitialValues } from "@/constants/form";
...
import {
  useBreakoutSessionInsert,
  useBreakoutSessionUpdate,
  useGetSessionById,
  useSetLocation,
...
interface BreakoutRoomDetailSessionFormProps {
  roomId: string;
}

const BreakoutRoomDetailSessionForm: FC<BreakoutRoomDetailSessionFormProps> = ({
  roomId,
}) => {
  ...
  const [initialValues, setInitialValues] =
    useState<SessionFormInitial>(SessionInitialValues);
  ...
  const { mutationAddBreakoutSession, data: addedBreakoutSession } =
    useBreakoutSessionInsert();
  const { mutationEditBreakoutSession } = useBreakoutSessionUpdate();
  const { fetchUsersByRole, data: users } = useListUsersByRole();
  const { mutationSetLocation } = useSetLocation(0, "", 0);
  ...
  const { fetchSessionById, data: aSession } = useGetSessionById();
  ...
  }, [fetchSessionById, slug]);

  useEffect(() => {
    if (slug !== undefined && slug !== "new") {
      if (aSession !== undefined) {
        const startDate = dayjs(
          aSession.getSessionById.startDate,
        ...
        const endDate = dayjs(
          aSession.getSessionById.endDate,
          "YYYY-MM-DDTHH:mm:ss.000Z"
        ).format("DD/MM/YYYY");
        ...
        setInitialValues({
          ...aSession.getSessionById,
          startDate: startDate,
          endDate: endDate,
          startTime: startTime,
          endTime: endTime,
          timezoneStart: aSession.getSessionById.offsetStart,
          timezoneEnd: aSession.getSessionById.offsetEnd,
        });
      }
    } else {
      setInitialValues(SessionInitialValues);
    }
  }, [aSession, slug]);
  ...
  useEffect(() => {
    if (addedBreakoutSession !== undefined && addedBreakoutSession !== null) {
      mutationSetLocation({
        variables: {
          setLocationInput: {
            sessionId: addedBreakoutSession.id,
            locationId: roomId,
          },
        },
      });
    }
  }, [addedBreakoutSession, mutationSetLocation, roomId]);
  ...
  const submitForm = useCallback(
    async (values: AuditoriumFormInitial) => {
      ...
      if (slug !== undefined && slug !== "new") {
        ...
        await mutationEditBreakoutSession({
          variables: {
        ...
        console.log({ properValues });
        await mutationAddBreakoutSession({
          variables: {
...
export default BreakoutRoomDetailSessionForm;
